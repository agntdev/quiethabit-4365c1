...
export class ChatDO {
  ...
  async alarm(): Promise<void> {
    ...
    for (const r of due) {
      try { await tg(this.env.BOT_TOKEN, "sendMessage", { chat_id: r.chatId, text: r.text, reply_markup: r.replyMarkup }); } catch { /* a blocked chat must not stop other reminders */ }
    }
